    ...
    <header className="flex h-11 items-center justify-between border-b border-border/50 bg-card/80 backdrop-blur-sm px-3 flex-shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5 rounded-md border border-border/60 bg-muted/40 px-2.5 py-1 text-sm">
          <span
            ...
            style={{ backgroundColor: selectedApp?.color ?? '#6366f1' }}
          />
          <span className="max-w-[160px] truncate font-medium text-foreground/90">
            {selectedApp?.name ?? 'Loading...'}
          </span>
      ...
      <div className="flex items-center gap-1.5">
        <Button
          ...
          title={forceError ? 'Disable error simulation' : 'Simulate API error'}
          className={forceError ? 'text-red-400 hover:text-red-300' : 'text-muted-foreground'}
        >
